...
contract discounter is Ownable {
  address public abcToken;
  mapping(address => bool) public claimed;
  bytes32 public merkleRoot = 0x6e47c00c277453ea5e7a8303c8ecb5530621271acf05c832adb90eafbf3d3841;
...
contract ERC721RG is randomizedABC, ERC721("Fruits2", "FRS2"), discounter {
  using SafeMath for uint256;

  mapping(address => bool) public canEarlyMint;
  ...
  function setEarlyMintFor(address _user, bool _withEarlyMint) public onlyOwner {
    canEarlyMint[_user] = _withEarlyMint;
  }
  ...
  function publicMint(
    ...
    require(_launched || canEarlyMint[msg.sender], "Not launched!");
    ...
    if (_proofOfDiscount[0] == bytes32(0)) {
      ...
      if (chosenGroupForDiscount == 1) {
        ...
        discountedPercentage = uint256(100).sub(msg.value.sub(_payedAmount).mul(100).div(A_price));
      }

      if (chosenGroupForDiscount == 2) {
        bool jumped;
        uint256 _payedAmount = msg.value;
        for (uint256 i = 0; i < _groups.length; i++) {
        ...
        discountedPercentage = uint256(100).sub(msg.value.sub(_payedAmount).mul(100).div(B_price));
      }

      if (chosenGroupForDiscount == 3) {
        bool jumped;
        uint256 _payedAmount = msg.value;
        for (uint256 i = 0; i < _groups.length; i++) {
        ...
        discountedPercentage = uint256(100).sub(msg.value.sub(_payedAmount).mul(100).div(C_price));
      }
      require(verifyClaim(msg.sender, discountedPercentage, _proofOfDiscount), "not eligible for a discount");
    }
    distributeFunds();
  }
